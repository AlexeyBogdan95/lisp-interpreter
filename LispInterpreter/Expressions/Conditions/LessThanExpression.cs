...
public class LessThanExpression: IExpression
{
    ...
    public ExpressionResult Execute(params string[] operands)
    {
        ...
        {
            throw new InvalidExpressionException("LessThan expression requires two operands");
        }
        ...
        {
            return ExpressionResult.CreateValueResult(left < right ? ConditionResultReturnValues.True : ConditionResultReturnValues.False);
        }

        throw new InvalidExpressionException("Operands of LessThan expression should be numbers");
    }
}